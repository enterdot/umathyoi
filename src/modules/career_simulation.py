...
class TurnConfiguration:
    ...
    def get_skill_count(self, skill_type: SkillType) -> int:
        pass
...
class TrainingEffect:
    ...
    def _make_effect_handlers(self):
        ...
        return {
            ...
            # sample card: 30107-maruzensky
                lambda vals: (
                    CardEffect(vals[0]),
                    self.turn_config.facility_levels.get(self.facility_type) * vals[1]
                ),
            ...
            # Not used in simulator - return null effect
                lambda vals: None,
            ...
            # sample card: 30102-el-condor-pasa
                lambda vals: (
                    CardEffect(vals[0]),
                    (self.turn_config.facility_cards[self.facility_type] - 1) * vals[1]
                ),
    ...
    def effects(self) -> dict[CardEffect, int]:
        ...
        for unique_effect_type, unique_effect_values in unique_effects:
            if unique_effect_type.value() < CardConstants.COMPLEX_UNIQUE_EFFECTS_ID_THRESHOLD:
                ...
                flattened_effects[CardEffect(unique_effect_type.value())] += unique_effect[0]
            else
                # Unique effect requires parsing
                # using the map above
        
        # Merge effects and flattened effects with appropriate logic
