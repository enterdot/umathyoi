...
class UmaMusumeCalculator:
    ...
    def __init__(self):
        ...
        # Mood lookup table
        self.mood_data = {"Great": 1.04, "Good": 1.02, "Normal": 1.0, "Bad": 0.98, "Awful": 0.96}

        # Style aptitude lookup table
        self.style_aptitude_data = {"S": 1.1, "A": 1.0, "B": 0.85, "C": 0.75, "D": 0.6, "E": 0.4, "F": 0.2, "G": 0.1}
    ...
    def calculate_stamina_needed(self):
        ...
        E42 = min((early_speed - start_end) / early_accel, (-start_end + math.sqrt(start_end**2 + 2 * early_accel * (self.distance / 6 - start_dist))) / early_accel)
